...
public class ShowIp extends Command {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if(deleteipconfig.getSection(ip) != null && deleteipconfig.getSection(ip).getBoolean("isToDelete")) {
        ...
        if(ipfileloaded) {
            ...
            if(!deleteipconfig.getSection(ip).getBoolean("isToDelete") || deleteipconfig.getSection(ip) == null) {
                for (String player : players) {
                    String player1 = UUIDFetcher.getName(player);
                    //ProxiedPlayer player1 = ProxyServer.getInstance().getPlayer(player);
                    if(player1 != null) {

                        p.sendMessage(messager("&9-&3" + player1));
                    }
                }
            }else{
